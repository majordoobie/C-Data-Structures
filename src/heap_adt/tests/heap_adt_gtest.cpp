 ...
 * print function
 */
void print_test(void *payload) { printf("%d\n", *(int *)payload); }
 ...
 */
void payload_destroy(void *payload) { free(payload); }
 ...
 * The first group is a pointer mode heap_adt while the second is a data mode
 * heap_adt.
 */
...
public:
  ...
  std::vector<int32_t> test_values = {1,  3,  4,  5,  7,  8,  9,   10,  14, 16,
                                      20, 23, 34, 56, 78, 90, 100, 121, 144};

  int get_min() { return test_values.front(); }
  int get_max() { return test_values.back(); }

protected:
  void SetUp() override {
    // Create the two ptr mode heaps and insert the values into it
    max_heap_ptr =
        heap_init(MAX_HEAP, HEAP_PTR, 0, payload_destroy, heap_ptr_cmp);

    min_heap_ptr =
        heap_init(MIN_HEAP, HEAP_PTR, 0, payload_destroy, heap_ptr_cmp);

    max_heap_data =
        heap_init(MAX_HEAP, HEAP_MEM, sizeof(int32_t), nullptr, heap_data_cmp);
    min_heap_data =
        heap_init(MIN_HEAP, HEAP_MEM, sizeof(int32_t), nullptr, heap_data_cmp);
...
TEST_F(HeapTestFixture, PrintFixtures) { heap_print(max_heap_ptr, print_test); }
...
TEST(HeapSortFind, HeapFindTestPtrModeMax) {
  ...
  int *val = (int *)heap_find_nth_item(int_ptr_array, array_length, 0, 5,
                                       HEAP_PTR, MAX_HEAP, heap_ptr_cmp);
