...
bst_compare_t compare(my_structure *current_payload,
                      my_structure *new_payload) {
...
void print(my_structure *node_payload) { printf("%d", node_payload->value); }
...
void free_payload(my_structure *node_payload) { free(node_payload); }
...
START_TEST(rotation_test_insert_leaf) {
  ...
  bst_traversal(tree, TRAVERSAL_PRE_ORDER, save_nodes, iter_obj);
  for (int i = 0; i < iter_obj->count; i++) {
    ck_assert_int_eq(iter_obj->my_struct_list[i]->value,
                     values_pre_order_before_rotation[i]);
  }
 ...
 */
static TFun rotation_tests[] = {rotation_test_insert_leaf,
                                rotation_test_big_data, NULL};

static TFun traversal_bst_test[] = {traversal_order_test, NULL};

static TFun fetch_tests[] = {fetch_existing_node, fetch_non_existing_node,
                             fetch_existing_update_node, fetch_exists_recursion,
                             NULL};
static TFun create_destroy_test_list[] = {tree_creation_not_null,
