...
/*!
 * @brief Parse the command line arguments and set the appropriate options in
 * the arg_parse_args_t struct. It will also add the file path array to the
 * struct
...
int8_t parse_args(int argc, const char **argv, file_tracker_t *args) {
  ...
  for (int index = optind; index < argc; index++) {
    if (file_exists(argv[index])) {
      args->files[file_path_index] =
          malloc(sizeof(char) * strlen(argv[index]) + 1);
      strncpy(args->files[file_path_index], argv[index],
              strlen(argv[index]) + 1);
