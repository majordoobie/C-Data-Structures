...
/*!
 * @brief Function creates a intersect_t object which is just a dictionary with
 * a file counter
...
intersect_t *create_intersect_dict() {
  ...
  intersect->dict =
      init_dict(intersect_dict_compare, intersect_free_payload, reduce_search);
  return intersect;
}

/*!
 * @brief Function is used to eliminate the the nodes from the tree that did not
 * make it to the second round. For example, if file one added 5 words and file
 * 2 only matched on 3 words, then the 2 words from the first iteration are
 * removed.
...
bst_recurse_t reduce_search(key_val_t *node_payload, void *ptr) {
  ...
  if (node_payload->file_count == list->target) {
    ...
    list->reduce_list[list->length] =
        create_intersect_payload(node_payload->original, 0);
    list->length++;
    return RECURSE_TRUE;
  }
}
...
/*!
 * @brief Checks the tree to see what nodes fail the iteration. If they fail,
 * add it to list of nodes to get deleted. Afterwards, iterate over the nodes
 * and delete them.
...
/*!
 * @brief Print the root node, the call the variation print to print any
 * children
...
/*!
 * @brief This function over writes the default behaviour of the bst_insert this
 * makes it possible to avoid inserting nodes that we do not need like repeated
 * nodes or adding the children of variations of the string output
 * @param payload[in] key_val_t
...
bst_status_t intersect_adv_add(key_val_t *payload, void *ptr, void *ptr2) {
  ...
  if (NULL == new_payload) {
    ...
    if (payload->file_count == new_payload->file_count) {
      ...
      // TODO: ERROR IS HERE - creates a verification and adds it I can see it
      // in memory
      //  but is new_payload lost?

      // increment the filecout on the string to indicate that "this file" has
...
/*!
 * @brief Creates the main nodes that go into the tree map. These will store the
 * first matched word and any other variations of it if there is a match
 * @param word
 * @param file_count
 * @return
 */
key_val_t *create_intersect_payload(const wchar_t *word, uint32_t file_count) {
  ...
  if (1 == file_count) {
    payload->variations =
        init_dict(intersect_dict_compare, variation_free_payload, NULL);
...
bst_compare_t intersect_dict_compare(key_val_t *current_payload,
                                     key_val_t *new_payload) {
  // reset collate just in case
  setlocale(LC_COLLATE, "en_US.UTF-8");

  int result =
      wcscmp(current_payload->collation_value, new_payload->collation_value);
...
/*!
 * @brief Create a variation key_val. The only difference from a intersection
 * key_val is that the collation key is not casted to uppercase, it maintains
 * its original string
