...
static node_t *
insert_node(node_t *node, node_payload_t *payload, bst_replace_t replace,
            bst_status_t (*callback)(node_payload_t *, void *, void *),
            void *ptr, bst_t *bst);
static node_t *get_node(bst_t *bst, node_t *node,
                        node_payload_t *target_payload);
static node_t *remove_node(node_t *node, node_payload_t *payload, bst_t *bst);
static void free_all_nodes(bst_t *bst, node_t *node,
...
static void print_2d_iter(node_t *node, int space,
                          void (*callback)(node_payload_t *));
static bst_recurse_t
in_order_traversal_node(node_t *node, bst_recurse_t recurse,
                        bst_recurse_t (*function)(node_payload_t *, void *),
                        void *void_ptr);
static bst_recurse_t
pre_order_traversal_node(node_t *node, bst_recurse_t recurse,
                         bst_recurse_t (*function)(node_payload_t *, void *),
                         void *void_ptr);
static bst_recurse_t
post_order_traversal_node(node_t *node, bst_recurse_t recurse,
                          bst_recurse_t (*function)(node_payload_t *, void *),
                          void *void_ptr);

/*!
 * @brief Public function used to printout the binary tree in a way that can
 * visualized. The functions takes a callback functions used to pass the actual
 * payloads to be printed.
 *
...
/*!
 * @brief Initialize the tree structure to include the payload file_count of the
 * node keys. The bst structure also will also require two callback functions.
 * One function to perform comparisons and another function to free the
 * payloads.
 *
 ...
 */
bst_t *bst_init(bst_compare_t (*compare)(node_payload_t *, node_payload_t *),
...
/*!
 * @brief Public function to destroy the binary tree with the option to either
 * free the payloads or leave them alone.
 * @param bst[in] bst_t
 * @param free_payload[in] Flag used to either destroy or preserve the payloads
 * in the nodes. The "True" will call the payload_free callback used when
 * registering the bst
 */
...
/*!
 * @brief Public function used to remove a node with the option to either
 * preserver or free the payload itself.
 *
 * @param bst[in] bst_t
...
/*!
 * @brief Create a new node with the given node_payload. Iterate over the tree
 * using the compare function supplied to the BST object and push the new node
 * onto the tree.
 *
 ...
 */
bst_status_t
bst_insert(bst_t *bst, node_payload_t *payload, bst_replace_t replace,
           bst_status_t (*callback)(node_payload_t *, void *, void *),
           void *ptr) {
 ...
 *
 * @param bst[in] bst_t
 * @param payload[in] node_payload_t You must create a payload with at least the
 * collation_value data initialized to do the comparisons
...
/*!
 * @brief Function that calls the internal traversal function and calls the
 * callback function passed to each node to be processed.
 *
 * @param bst[in] bst_t
 * @param type[in] Type of traversal
 * @param callback[in] Pointer to external supplied function for node processing
 */
void bst_traversal(bst_t *bst, bst_traversal_t type,
...
/*!
 * @brief Insert the payload into a newly created node and add it to the tree.
 * If the node already exists, check if the replace flag is set to True, if so,
 * replace payload in the node.
 *
 * Additionally, if more granular control is needed for adding matched nodes,
 * you have the option of providing a callback function along with a void
 * pointer to whatever structure is needed
 *
 * @param node[in] node_t
 * @param payload[in] node_payload_t
 * @param replace[in] bst_replace_t
 * @param callback[in] Pointer to a callback function for the option to do with
 * a already matched node
 * @param ptr[in] NULL pointer for using callback
 * @param bst[in] bst_t
 * @return Return the newest root node. This will change if a rotation is needed
...
insert_node(node_t *node, node_payload_t *payload, bst_replace_t replace,
  ...
  if (BST_LT == result) {
    node->left_child =
        insert_node(node->left_child, payload, replace, callback, ptr, bst);
  } else if (BST_GT == result) {
    node->right_child =
        insert_node(node->right_child, payload, replace, callback, ptr, bst);
...
/*!
 * @brief Private iterative function to find the node payload that matches the
 * payload passed in.
 *
 * @param bst[in] bst_t
 * @param node[in] node_t
...
/*
 * @brief Recursively move through the nodes until a match is found. Once it is
 * found, return the node to be freed.
 *
 * @param node[in] node_t
 * @param payload[in] node_payload_t
 * @param bst[in] bst_t
...
static node_t *remove_node(node_t *node, node_payload_t *payload, bst_t *bst) {
  ...
  if (BST_LT == result) {
    ...
    /*
     * The following two IF statements check if the left or right node are NULL.
     * If they are, return the opposite. Returning will assign that node to one
     * of the two if statements above i.e. "node->right_child = remove();"
     */
    if ((NULL == node->left_child) || (NULL == node->right_child)) {
       ...
       * either child node of the current node is NULL, it will return the
       * opposite making that node the new child of one of the parent nodes
       * above.
       *
       ...
       * subtree. The approach will instead check the height to make sure the
       * tree is balanced.
 ...
 * @param bst[in] bst_t
 * @param node[in] node_t
 * @param free_payload[in] bst_status_t should include either FREE_PAYLOAD_FALSE
 * or TRUE
 */
static void free_all_nodes(bst_t *bst, node_t *node,
                           bst_destroy_t free_payload) {
...
/*!
 * @brief Function uses node heights to know when and how to rotate the tree to
 * balance it
 *
 * @param node[in] node_t
...
/*!
 * @brief Private function to get the height of the current node while checking
 * for nulls
 *
 * @param node[in] node_t
...
static void set_height(node_t *node) {
  node->height =
      node_max(get_height(node->left_child), get_height(node->right_child)) + 1;
}
...
/*!
 * @brief Iterate through the right side of given node until NULL is reached
 * then return the collation_value of that node
 * @param node[in] node_t
 * @return Returns the node_payload_t of the last node on the right side of a
 * node given
 */
static node_t *find_max_payload(node_t *node) {
...
/*!
 * @brief Function to recursively printout the nodes in a manner that you can
 * visually see in the terminal
 * @param node[in] node_t
...
in_order_traversal_node(node_t *node, bst_recurse_t recurse,
  ...
  if (RECURSE_TRUE == recurse) {
    recurse =
        in_order_traversal_node(node->right_child, recurse, function, void_ptr);
  }
...
pre_order_traversal_node(node_t *node, bst_recurse_t recurse,
  ...
  if (RECURSE_TRUE == recurse) {
    recurse =
        pre_order_traversal_node(node->left_child, recurse, function, void_ptr);
  }

  if (RECURSE_TRUE == recurse) {
    recurse = pre_order_traversal_node(node->right_child, recurse, function,
                                       void_ptr);
  }
  return recurse;
}
...
post_order_traversal_node(node_t *node, bst_recurse_t recurse,
  ...
  if (RECURSE_TRUE == recurse) {
    recurse = post_order_traversal_node(node->left_child, recurse, function,
                                        void_ptr);
  }
  if (RECURSE_TRUE == recurse) {
    recurse = post_order_traversal_node(node->right_child, recurse, function,
                                        void_ptr);
  }
  if (RECURSE_TRUE == recurse) {
    recurse = function(node->key, void_ptr);
