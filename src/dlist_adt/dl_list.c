...
static void dlist_destroy_(dlist_t **dlist, dlist_settings_t free_nodes,
                           void (*free_func)(void *));
static dnode_t *init_node(void *data);
static void *remove_node(dlist_t *dlist, dnode_t *node);
static dlist_result_t add_node(dlist_t *dlist, void *data,
                               dlist_settings_t add_mode, int32_t at_index);
...
dlist_t *dlist_init(dlist_match_t (*compare_func)(void *, void *)) {
  ...
  *iter_list = (dlist_t){
      .compare_func = compare_iters, .iter_list = NULL, .is_iter_mgr = true};

  *dlist = (dlist_t){.compare_func = compare_func,
                     .iter_list = iter_list,
                     .is_iter_mgr = false};
 ...
 */
size_t dlist_get_length(dlist_t *dlist) { return dlist->length; }
...
void *dlist_remove_value(dlist_t *dlist, void *data) {
  ...
  if ((false == dlist->is_iter_mgr) && (false == is_iter_list_empty(dlist))) {
    iter_search_t *tail_node =
        iter_search_by_value_plus(dlist, dlist->tail->data);
    iter_search_t *head_node =
        iter_search_by_value_plus(dlist, dlist->head->data);
...
void dlist_quick_sort(dlist_t *dlist, sort_direction_t direction,
  ...
  *sort = (quick_sort_t){.compare_func = compare_func, .direction = direction};
...
dlist_iter_t *dlist_get_iterable(dlist_t *dlist, iter_start_t pos) {
  ...
  dlist_iter_t *iter = iter_get_iterable(
      (ITER_HEAD == pos) ? dlist->head : dlist->tail, dlist,
      (ITER_HEAD == pos || 0 == dlist->length) ? 0
 ...
 * @param free_func
 */
static void dlist_destroy_(dlist_t **dlist_ref, dlist_settings_t free_nodes,
                           void (*free_func)(void *)) {
  if (NULL == dlist_ref) {
    return;
  }
