...
dlist_iter_t *iter_get_iterable(dnode_t *node, dlist_t *dlist, int32_t index) {
  ...
  *iter = (dlist_iter_t){.node = node, .dlist = dlist, .index = index};
 ...
 * @param iter
 */
void iter_destroy_iterable(dlist_iter_t *iter) { free(iter); }
 ...
 *
 * @param iter
 * @return Pointer to the void data stored at the iter node. If NULL is
 * returned, then the dlist is empty
 */
 ...
 */
int32_t iter_get_index(dlist_iter_t *iter) { return iter->index; }
 ...
 */
dnode_t *iter_get_node(dlist_iter_t *iter) { return iter->node; }
 ...
 */
dlist_t *iter_get_dlist(dlist_iter_t *iter) { return iter->dlist; }
...
void iter_update_index(dlist_iter_t *iter, int index) { iter->index += index; }
 ...
 */
iter_search_t *iter_init_search(dlist_iter_t *iter, void *data, int32_t index,
 ...
 * @param search
 */
void iter_destroy_search(iter_search_t *search) { free(search); }
 ...
 * is capable of conversing a negative value into the proper index. If a
 * negative is used then a reverse iteration is performed.
 *
 * @param search
...
iter_search_result iter_search(iter_search_t *search) {
  ...
  while (NULL != current_node) {
    if (SEARCH_BY_VALUE == search->search_by) {
      if (DLIST_MATCH ==
          compare_func(current_node->data, search->target_data)) {
