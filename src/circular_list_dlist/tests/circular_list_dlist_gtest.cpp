...
char *get_payload(const char *string) { return strdup(string); }

// Function to free the payloads
void free_payload(void *node) { free(node); }
...
protected:
  void SetUp() override {
    this->clist =
        clist_init((uint32_t)this->words.size(), match_payloads, free_payload);
    for (std::string &val : this->words) {
      clist_insert(this->clist, get_payload(val.c_str()), 0, TAIL);
    }
  }
  void TearDown() override { clist_destroy(this->clist, FREE_NODES_TRUE); }
};
...
TEST_F(CListTestFixture, TestFindingNode) {
  ...
  char *target_node =
      (char *)clist_find(this->clist, (void *)node_to_find.c_str());
...
TEST_F(CListTestFixture, TestNodeRemoval) {
  std::string node_to_find = this->words.at(this->words.size() / 2);
  char *target_node =
      (char *)clist_remove(this->clist, (void *)node_to_find.c_str());
