...
public:
  ...
  std::vector<std::string> keys = {
      ...
      std::string("Seventh"), std::string("Eight"),  std::string("9"),
      std::string("10"),      std::string("11"),     std::string("12"),
  };

protected:
  void SetUp() override {
    dict = htable_create(hash_callback, compare_callback, NULL, free_payload);
...
TEST_F(HashtableGtest, TestKeyExists) {
  EXPECT_EQ(true,
            htable_key_exists(this->dict, (void *)this->keys.at(0).c_str()));

  EXPECT_EQ(true,
            htable_key_exists(this->dict, (void *)this->keys.at(7).c_str()));
  EXPECT_EQ(true,
            htable_key_exists(this->dict, (void *)this->keys.at(9).c_str()));
  EXPECT_EQ(true,
            htable_key_exists(this->dict, (void *)this->keys.at(11).c_str()));
...
TEST_F(HashtableGtest, TestValueFreeOnUpdate) {
  ...
  void *old_payload =
      htable_set(this->dict, (void *)first_key.c_str(), new_payload);
...
TEST_F(HashtableGtest, TestKeyRemoval) {
  ...
  std::string first_key = this->keys.at(0);
  void *payload =
      htable_del(this->dict, (void *)first_key.c_str(), HT_FREE_PTR_FALSE);
...
TEST_F(HashtableGtest, TestKeyAllRemoval) {
  ...
  while (NULL != entry) {
    void *payload =
        htable_del(this->dict, (void *)entry->key, HT_FREE_PTR_FALSE);
    free_payload(payload);
...
TEST(HashtableSoloTest, TestExpansion) {
  htable_t *dict =
      htable_create(hash_callback, compare_callback, NULL, free_payload);
...
void free_payload_dl(void *data) { free(data); }
...
TEST(HashtableBenchMark, BenchMark) {
  ...
  htable_t *table =
      htable_create(hash_callback, compare_callback, NULL, free_payload_dl);
