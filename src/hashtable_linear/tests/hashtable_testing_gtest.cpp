...
class HashtableGtest : public ::testing::Test
{
 ...
 protected:
    void SetUp() override
    {
        ...
        for (auto& key: keys)
        {
            htable_set_str(dict, key.c_str(), HT_KEY_AS_STR, get_payload(key));
        }
    }
...
TEST(HashtableSoloTest, TestExpansion)
{
    ...
    htable_set_str(dict, "Key1", HT_KEY_AS_STR, get_payload("Key1"));
    htable_set_str(dict, "Key2", HT_KEY_AS_STR, get_payload("Key2"));
    htable_set_str(dict, "Key3", HT_KEY_AS_STR, get_payload("Key3"));
    htable_set_str(dict, "Key4", HT_KEY_AS_STR, get_payload("Key4"));
    ...
    // our used slots by 1
    htable_set_str(dict, "Key5", HT_KEY_AS_STR, get_payload("Key5"));
    EXPECT_EQ(4, htable_get_length(dict));
    ...
    // copied over
    htable_set_str(dict, "Key6", HT_KEY_AS_STR, get_payload("Key6"));
