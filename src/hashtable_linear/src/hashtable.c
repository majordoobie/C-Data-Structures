...
typedef enum { VALID_PTR = 1, INVALID_PTR = 0 } valid_ptr_t;
...
static htable_entry_t *get_entry(htable_t *table, void *key, uint64_t *slot);
 ...
 */
uint64_t htable_get_init_hash(void) { return (uint64_t)FNV_OFFSET_BASIS; }
...
htable_t *htable_create(uint64_t (*hash_callback)(void *),
  ...
  *table = (htable_t){.free_value = free_value_callback,
  ...
  table->entries =
      (htable_entry_t **)calloc(table->capacity, sizeof(htable_entry_t *));
  ...
  *slot = hash % (table->capacity - 1);
  ...
  while ((NULL != current_entry->key) || (true == current_entry->_dummy_key)) {
    if (false == current_entry->_dummy_key) {
      if (HT_MATCH_TRUE ==
          (table->compare_callback(key, table->entries[(*slot)]->key))) {
