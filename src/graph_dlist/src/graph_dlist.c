...
static edge_t *create_edge(gnode_t *from_node, gnode_t *to_node,
                           uint32_t weight);
...
static void get_hex_value(void *ptr, char *string, size_t buff_size) {
  snprintf(string, buff_size, "%02hhX%02hhX", ((unsigned char *)&ptr)[1],
           ((unsigned char *)&ptr)[0]);
}
...
void graph_print(graph_t *graph, char *(node_data_repr(void *))) {
  ...
  puts("<#>[#] ->:    Node value if available followed by the last two bytes "
       "of the pointer");
  puts("-> <#>(#)[5]: Node value if available followed by the last\n"
       "              2 bytes of the pointer followed by the weight of the "
       "edge");
 ...
 */
void *graph_get_node_value(gnode_t *node) { return node->data; }
 ...
 */
graph_opt_t graph_remove_node(graph_t *graph, gnode_t *node,
                              void(free_func(void *))) {
...
gnode_t *graph_create_node(void *data) {
  ...
  *node = (gnode_t){.data = data, .edges = dlist};
 ...
 */
size_t graph_edge_count(gnode_t *node) { return dlist_get_length(node->edges); }
 ...
 * graph
 */
graph_opt_t graph_add_edge(graph_t *graph, gnode_t *source_node,
                           gnode_t *target_node, uint32_t weight) {
  assert(graph);
  assert(source_node);
  assert(target_node);
...
bool graph_edge_in_graph(graph_t *graph, edge_t *edge) {
  ...
  // Grab all the source_node_edges from the source node of the edge we are
  // looking for
 ...
 */
path_t *graph_get_path(graph_t *graph, gnode_t *source_node,
                       gnode_t *target_node) {
  // First make sure that both nodes are already in the graph
  if (!(graph_node_in_graph(graph, source_node)) ||
      !(graph_node_in_graph(graph, target_node))) {
  ...
  heap_t *heap = heap_init(MIN_HEAP, HEAP_PTR, 0, NULL, heap_ptr_cmp);

  // Table to quickly map gnode_t -> dij_node_t
  htable_t *dij_lookup_table =
      htable_create(graph->hash_callback,
                    (htable_match_t(*)(void *, void *))graph->compare_callback,
                    NULL, free_dijkstra_node);

  // Table for marking the visited items
  // Table to quickly map gnode_t -> gnode_t
  htable_t *visited_table =
      htable_create(graph->hash_callback,
                    (htable_match_t(*)(void *, void *))graph->compare_callback,
                    NULL, free_dijkstra_node);
  ...
  while (!heap_is_empty(heap)) {
    ...
    htable_set(visited_table, (void *)curr_dij_node->self, curr_dij_node->self);
    ...
    while (NULL != neighbor) {
      ...
      dijkstra_t *neigh_dij_node =
          (dijkstra_t *)htable_get(dij_lookup_table, (void *)neighbor->to_node);
       ...
       * to the source
       */
      if (neigh_dij_node->distance >
          (curr_dij_node->distance + neighbor->weight)) {
...
static dijkstra_t *get_dijkstra_node(gnode_t *self_node) {
  ...
  *node = (dijkstra_t){.distance = UINT32_MAX, .self = self_node, .prev = NULL};
  return node;
}

static void free_dijkstra_node(void *node) { free(node); }
...
static edge_t *create_edge(gnode_t *from_node, gnode_t *to_node,
  ...
  *edge =
      (edge_t){.from_node = from_node, .to_node = to_node, .weight = weight};
  return edge;
}
...
static void free_edge_dnode(void *node) { free(node); }
