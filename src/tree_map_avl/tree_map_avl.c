...
/*!
 * @brief Insert a key_val_t into the dictionary. If successful, return a 1 otherwise 0.
...
{
    bst_status_t status = bst_insert(dict->tree, key_val, REPLACE_PAYLOAD_TRUE, NULL, NULL);
...
/*!
 * @brief If more granular control is needed for how to handle when nodes are found in
 * the tree you have the option of supplying a callback function for the node and also
 * providing a void pointer to whatever structure is needed to be successful.
 * @param dict
 * @param key_val
 * @param callback
 * @param ptr
...
/*!
 * @brief Function calls upon the traversal function of the backend AVL tree and process
 * each node with the given "value_search" function. Recursion stops when the provided
 * functions instructs it to stop. Nothing is returned. For this reason, it is wise to use
 * the void ptr for returning back any values you may wish.
 * @param dict[in] dict_t
