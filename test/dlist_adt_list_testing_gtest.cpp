...
TEST(dlist_test, IterableTest) {
  ...
  std::vector<char *> vector = {get_payload(1), get_payload(2), get_payload(3)};
...
// Test that removing a single node on a single node iter sets the pointer to
// null
...
TEST_F(DListTestFixture, TestUpdatingItersAfterRemoval) {
  ...
  char *previous_payload =
      (char *)test_vector.at((size_t)(test_vector.size() - 2)).c_str();
